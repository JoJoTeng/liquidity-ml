"""
Step 3: Standard ML Is Affected (ElasticNet version)
======================================================
...
Outputs (saved to outputs/motivation_raw/step3_elasticnet/{liquidity}/):
  Same structure as 07, with _elasticnet suffix in metadata.

Usage:
  python scripts/08_step3_elasticnet.py
  python scripts/08_step3_elasticnet.py --liquidity mcap
  python scripts/08_step3_elasticnet.py --recompute
"""
...
def main():
    parser = argparse.ArgumentParser(description="Step 3: ElasticNet Diagnostics")
    parser.add_argument(
        "--liquidity",
        type=str,
        default="dvol",
    ...
    parser.add_argument(
        "--recompute",
        action="store_true",
        help="Skip training; use saved predictions and importances",
    )
    ...
    if missing_feats:
        logger.warning("Features not in panel: %s", missing_feats)
    ...
    # ══════════════════════════════════════════════════════
    # Phase 1: Train ElasticNet (or load saved predictions)
    # ══════════════════════════════════════════════════════
    ...
    if args.recompute and pred_path.exists() and imp_path.exists():
        ...
        logger.info("=" * 60)
        logger.info("Phase 1: Rolling ElasticNet training...")
        predictions, importances = rolling_elasticnet_predict(panel, features, config)
    ...
    # ══════════════════════════════════════════════════════
    # Phase 2: Compute diagnostics (same as 07)
    # ══════════════════════════════════════════════════════
    avg_importance = importances.mean()

    # ── Output 3.1 ──
    logger.info("=" * 60)
    logger.info("Output 3.1: Feature importance (|coef|) vs illiquidity-relatedness")
    ...
    q_r2_tex.to_latex(
        tex_dir / "R2ByQuintileElasticNet.tex",
        index=False,
        escape=False,
    ...
    logger.info(
        "CS-mean: Std R²=%.4f%%, Wtd R²=%.4f%%, Gap=%.4f pp",
    ...
    # ── Summary ──
    logger.info("=" * 60)
    logger.info("Step 3 (ElasticNet) complete. Outputs saved to %s", output_dir)
    logger.info("  3.1  importance_vs_illiquidity.png (ρ=%.3f)", rho_31)
    logger.info("  3.2  importance_vs_liquid_r2.png")
    logger.info("  3.3  r2_by_quintile_cs.png / r2_by_quintile_zero.png")
