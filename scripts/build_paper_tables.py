...
\caption{{\footnotesize \textbf{{Stock-level predictors.}} The $113$ characteristics used throughout the main paper, drawn from the open-source library of \citet{{chen2022open}} (see Section~3.1 of the main paper for the selection rule). Descriptions are the library's short names; full construction details are documented in the library. The category column reports the broad economic grouping used in Sections~2 and~3 of the main paper.}}
...
\bottomrule
\end{{tabular}}
\caption{{\footnotesize \textbf{{Training-scale sensitivity of the decomposition.}} Net annualised training effect ($2A-1A$, Panel~A) and total effect ($2B-1A$, Panel~B) of Equation~(15) of the main paper for the capacity portfolio, in annualised net Sharpe-ratio units, as the scale $A$ at which the training weight of Equation~(7) of the main paper is computed (rows) varies over the fitted grid of Section~3.3 of the main paper and the capital at which the portfolio is charged costs (columns) varies over the capital grid of Table~9 of the main paper; the main paper reports the \$500M row. Each row is a self-contained pipeline---the row's weight enters the training loss, the portfolio tilt, and the deployment-weighted evaluation---so effects are comparable across rows, while the underlying cell Sharpe ratios (not shown) are not. Matched training and deployment scales lie on the diagonal, with the Prop.~TC column (half-spread only) approximating the \$10M zero-impact limit. One-sided \citet{{ledoit2008robust}} bootstrap $p$-values in parentheses, from the same seeded machinery as Table~9 of the main paper, unadjusted across the sixteen cells of each panel. The total effect is positive in every cell and nominally significant at the $5\%$ level in {n_sig_txt} of sixteen ({n_bonf_txt} of sixteen survive a Bonferroni correction at the same level); no training effect is individually significant even before adjustment (the smallest $p$, ${min_train_p:.3f}$, is the unadjusted minimum over the grid), and Panel~A is read as a pattern of point estimates, not as cell-level inference, as in Table~12 of the main paper. 299 months, 2000--2024.}}
