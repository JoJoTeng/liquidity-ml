...
\bottomrule
\end{{tabular}}
\caption{{\footnotesize \textbf{{Gross-of-costs effects at the \$500M deployment scale.}} The decomposition of Equation~\eqref{{eq:decomp}} computed on gross rather than net Sharpe ratios, one row per specification of Section~\ref{{sec:results}}, supporting the gross-side reading of Section~\ref{{subsec:twobytwo_results}}. One-sided $p$-values in parentheses are from the same seeded \citet{{ledoit2008robust}} bootstrap; because it tests the positive tail, the primary gross total's $p=0.997$ is a rejection under the mirrored negative-tail test, and no gross effect is significantly positive anywhere in the design. Gross execution carries no separate pairwise test. 299 months, 2000--2024.}}
