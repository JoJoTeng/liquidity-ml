...
def build_consistency_dose_response():
    ...
    # Capacity-portfolio training effect quoted in the caption, read from the
    # eval-realignment cache rather than hard-coded.
    cap_tr = pd.read_csv(
        EVAL_ROOT / "xgboost/tc_rank_lam3_500m/two_by_two_500M.csv"
    ).set_index("metric")["value"]["Net training effect annualized"]
    return rf"""\begin{{table}}[t!]
\centering
\begin{{tabular}}{{lccc}}
\toprule
...
def build_weight_family_sweep():
    ...
    rows = []
    inter = []  # per-spec net interaction at $500M, quoted in the caption
    for spec, lab, scheme, c, aum in SPECS:
        ...
        inter.append(t["Net interaction annualized"])
...
\bottomrule
\end{{tabular}}
\caption{{\footnotesize \textbf{{Gross-of-costs effects at the \$500M deployment scale.}} The decomposition of Equation~\eqref{{eq:decomp}} computed on gross rather than net Sharpe ratios, one row per specification of Section~\ref{{sec:results}}, supporting the gross-side reading of Section~\ref{{subsec:twobytwo_results}}. One-sided $p$-values in parentheses are from the same seeded \citet{{ledoit2008robust}} bootstrap; because it tests the positive tail, the primary gross total's $p=0.997$ is a rejection under the mirrored negative-tail test, and across the Section-4 specifications at \$500M, no tested gross effect is significantly positive. Gross execution carries no separate pairwise test. 299 months, 2000--2024.}}
...
\bottomrule
\end{{tabular}}
\caption{{\footnotesize \textbf{{The conventional prediction-sorted portfolio.}} Monthly-refreshed portfolios sorted into quintiles of the return forecast on the full cross-section, equal-weighted within quintile, for the standard model and the primary weighted specification. Gross and Net are annualised mean returns in per cent; Net SR is the annualised net Sharpe ratio. Net returns charge the cost primitive of Equation~\eqref{{eq:tc_primitive}} at the \$500M deployment scale: each quintile row is a standalone long-only portfolio deploying the full \$500M, and the long--short row is the dollar-neutral $Q5{{-}}Q1$ portfolio with \$250M per leg, so the rows are separate portfolios rather than an additive decomposition. The gross ladder is monotone for both models, but at deployment scale the net premium survives only in the top quintile, and the long--short portfolio---gross annualised Sharpe ratio $1.66$ standard, $1.39$ weighted---is unprofitable net of costs for both. 299 months, 2000--2024.}}
...
\bottomrule
\end{{tabular}}
\caption{{\footnotesize \textbf{{Training $\times$ execution on the conventional sorted portfolio.}} The two-by-two design of Section~\ref{{subsec:twobytwo_results}} applied to the equal-weighted quintile long--short portfolio of Section~\ref{{subsec:conventional_results}}: training loss (standard vs.\ implementability-weighted) crossed with execution (plain monthly membership refresh vs.\ the cost-scaled membership-hysteresis band of Equation~\eqref{{eq:hysteresis}}). Panel~A reports the four cells at the primary \$500M scale: net and gross annualised Sharpe ratios, mean monthly cost drag in basis points, and monthly one-sided turnover. Panels~B and~C span the three specifications of Section~\ref{{sec:results}}; because the legs are equal-weighted, the standard cells ($1A$, $1B$) are identical across specifications and only the weighted rows vary. Panel~B decomposes the net gain at each level of deployed capital; one-sided $p$-values in parentheses are from the in-file \citet{{ledoit2008robust}} studentised circular-block bootstrap (training $2A$ vs.\ $1A$, execution $1B$ vs.\ $1A$, total $2B$ vs.\ $1A$); the $2B{{-}}1B$ adoption contrast is reported without a $p$-value, as the pairwise machinery of the inference supplement covers the capacity portfolios only. Effects are computed on unrounded values, so the printed identities can differ in the last digit. Panel~C reports annualised factor alphas of the four net return series at \$500M with Newey--West $t$-statistics (6 lags). 299 months, 2000--2024.}}
\label{{tab:conv_2x2}}
\end{{table}}
"""
