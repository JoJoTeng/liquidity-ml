"""Summarize the section 4.3 breakeven-gated capacity training effect into one workbook.

Mirror of make_capacity_training_effect_xlsx.py, but for the BREAKEVEN-GATED book:
reads the metrics CSVs written by 43_breakeven_capacity_portfolio.py, keeps the
``breakeven_gate`` rows, and writes a single plain .xlsx with one sheet per model.
Within each model sheet the results are stacked as three labelled panels covering
every fitted weight spec:
...
Effect sizes and gross levels only. Reads outputs, writes a workbook; nothing in
the analysis cache is modified.

    python scripts/eval_realignment/make_breakeven_training_effect_xlsx.py
"""
...
OUT = REPO / "outputs" / "eval_realignment" / "tables" / "capacity_breakeven_training_effect.xlsx"

# Which execution book in the section-4.3 metrics CSVs to report.
BOOK = "breakeven_gate"
...
def _metrics(model_dir: str, spec_dir: str, aum: str) -> pd.DataFrame | None:
    f = ANALYSIS / model_dir / spec_dir / f"capacity_breakeven_metrics_{aum}.csv"
    if not f.exists():
        return None
    d = pd.read_csv(f)
    d = d[d["book"] == BOOK]
