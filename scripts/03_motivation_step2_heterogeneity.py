...
Outputs (saved to outputs/motivation/step2/{liquidity_key}/):
...
With --full, the script also writes *_full files that repeat the Step 2
analyses using the final 113-feature universe from data/feature_list.json.

Usage:
  python scripts/03_motivation_step2_heterogeneity.py
  python scripts/03_motivation_step2_heterogeneity.py --liquidity mcap
"""
...
from src.data.loader import rank_to_unit_interval
...
def main():
    ...
    parser.add_argument(
        "--full",
        action="store_true",
        help="Also run full 113-feature versions of the Step 2 analyses",
    )
    ...
    panel["liq_rank"] = panel.groupby("yyyymm")[liq["quintile_col"]].transform(
    ...
    formatted.to_csv(output_dir / "quintile_fm_coefficients.csv", index=False)
    ...
    logger.info(
        "Features with significant Q5-Q1 difference: %d/%d", n_sig_diff, len(ct)
    )
    ...
    plot_quintile_coefficients(q_results, focal, output_dir / "coefficient_plots.png")
    ...
    logger.info("Dummy F-test p-value: %.4f", int_dummy["f_test_pvalue"])

    # LaTeX table (primary)
    tex_int = int_results["coef_table"].copy()
    tex_int = tex_int.rename(
        columns={
    ...
    step1_dir = Path(get_output_dir()) / "motivation" / "step1" / liquidity_key
    div_stats_path = step1_dir / "divergence_stats.csv"
    if not div_stats_path.exists():
        logger.warning(
            "divergence_stats.csv not found at %s. "
            "Run scripts/02_motivation_step1_divergence.py first. Skipping Output 2.4.",
            div_stats_path,
        )
    else:
    ...
    # ══════════════════════════════════════════════════════
    # Full 113-feature versions of Outputs 2.1--2.4
    # ══════════════════════════════════════════════════════
    if args.full:
        logger.info("=" * 60)
        ...
        if not feature_list_path.exists():
            logger.error(
                "feature_list.json not found! Run scripts/01_process_data.py first."
            )
            sys.exit(1)
        with open(feature_list_path) as f:
            feature_meta = json.load(f)
        all_features = (
            feature_meta["features"] if isinstance(feature_meta, dict) else feature_meta
        )
        ...
        logger.info(
            "Output 2.3b: Full interaction regression (%d features)", len(all_features)
        )
        int_full = interaction_fama_macbeth(panel_full, all_features, "liq_rank")
        int_full["coef_table"].to_csv(
            output_dir / "interaction_regression_full.csv", index=False
        )
        n_sig_full = int((int_full["coef_table"]["gamma_t"].abs() > 2).sum())
        logger.info(
            "Full: %d/%d sig γ, F-test p=%.6f",
            n_sig_full,
            len(all_features),
            int_full["f_test_pvalue"],
        )

        # Category summary
        broad_map = load_feature_categories()["broad"]
        ct_full = int_full["coef_table"].copy()
        ct_full["category"] = ct_full["feature"].map(broad_map).fillna("Other")
        cat_summary = (
            ct_full.groupby("category")
            .agg(
            ...
            .reset_index()
            .sort_values("n_sig_gamma", ascending=False)
        )
        ...
        # Robustness: dummy
        int_full_dummy = interaction_fama_macbeth(
            panel_full, all_features, "liq_rank", use_dummy=True
        )
        int_full_dummy["coef_table"].to_csv(
            output_dir / "interaction_regression_full_dummy.csv", index=False
        )
        ...
        q_full["coef_table"].to_csv(
            output_dir / "quintile_fm_coefficients_full_raw.csv", index=False
        )
        ...
        logger.info(
            "Saved formatted full quintile table (%d features)", len(q_full_fmt)
        )
        ...
        logger.info(
            "Output 2.2b: Coefficient plots for %d significant features",
            len(sig_features),
        )
        page_size = 20
        ...
        if div_stats_path.exists():
            ...
            df_scatter = df_scatter.dropna(subset=["abs_d_bar", "abs_gamma"])
            if len(df_scatter) < 3:
                logger.warning(
                    "Only %d matched full-feature scatter points; skipping full scatter.",
                    len(df_scatter),
                )
                rho_full = np.nan
            else:
                rho_full, _ = spearmanr(
                    df_scatter["abs_d_bar"], df_scatter["abs_gamma"]
                )
                ...
                ax.scatter(
                    df_scatter["abs_d_bar"],
                    df_scatter["abs_gamma"],
                    s=25,
                    color="steelblue",
                    alpha=0.6,
                    zorder=3,
                ...
                ax.set_ylabel(
                    r"$|\bar{\gamma}_j|$ (predictability heterogeneity)", fontsize=11
                )
                ax.set_title(
                    f"Distributional Divergence vs Predictability Heterogeneity\n"
                    f"(Spearman ρ = {rho_full:.3f}; N = {len(df_scatter)} features)",
                    fontsize=11,
                )
            ...
            df_scatter.to_csv(
                output_dir / "divergence_vs_heterogeneity_full.csv", index=False
            )
            logger.info(
                "Saved divergence_vs_heterogeneity_full.csv (%d features)",
                len(df_scatter),
            )
    ...
    logger.info(
        "  2.3  interaction_regression.csv (F-test p=%.4f)",
        int_results["f_test_pvalue"],
    )
    logger.info(
        "       interaction_regression_dummy.csv (F-test p=%.4f)",
        int_dummy["f_test_pvalue"],
    )
    if "spearman_rho" in meta:
        logger.info(
            "  2.4  divergence_vs_heterogeneity.png (ρ=%.3f)", meta["spearman_rho"]
        )
    if args.full:
        logger.info("  full interaction_regression_full.csv")
        logger.info("       interaction_regression_full_dummy.csv")
        logger.info("       interaction_by_category.csv")
        logger.info("       quintile_fm_coefficients_full.csv")
        logger.info("       quintile_fm_coefficients_full_raw.csv")
        logger.info("       coefficient_plots_full_p*.png")
        if "spearman_rho_full" in meta:
            logger.info(
                "       divergence_vs_heterogeneity_full.png (ρ=%.3f)",
                meta["spearman_rho_full"],
            )
