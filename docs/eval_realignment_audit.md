...
| §4.2 signal-weighted capacity portfolio | §4.2 | `src/analysis/eval_realignment/capacity_portfolio.py` (script 42) | **yes (built)** | implemented in the eval_realignment track; equal/value capacity-weighting benchmarks added alongside the signal book (`--capacity-weighting`); see `docs/eval_realignment_pipeline.md` §3 |
...
5. **Implementable-utility / Jensen optimizer (§4.5) — not pursued.** A Static-tier closed-form mean–variance optimizer was prototyped (former script 46) but dropped from this branch; the implementable mean–variance / efficient-frontier track is no longer part of the pipeline.
